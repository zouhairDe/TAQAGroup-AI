...
export class AnomalyImportHandler {
  ...
  private async processCSVFile(filepath: string, fileId: string): Promise<void> {
    try {
      ...
      try {
        // Preprocess the CSV file to fix common issues like line breaks in fields
        processedFilePath = await this.preprocessCsvFile(filepath);
        logger.info(`Preprocessed CSV file: ${filepath} -> ${processedFilePath}`);
      } catch (preprocessError) {
        logger.warn(`Failed to preprocess CSV file, continuing with original: ${preprocessError instanceof Error ? preprocessError.message : 'Unknown error'}`);
      }
      
      // Create a more resilient parser with enhanced error handling options      // Define a list of expected headers with common variations
      const expectedHeaders = {
        'numEquipement': ['numEquipement', 'num_equipement', 'Num Equipement', 'NumeroEquipement'],
        'description': ['description', 'Description', 'desc'],
        'dateDetectionAnomalie': ['dateDetectionAnomalie', 'date_detection_anomalie', 'Date Detection Anomalie', 'DateDetection'],
        'descriptionEquipement': ['descriptionEquipement', 'description_equipement', 'Description Equipement'],
        'sectionProprietaire': ['sectionProprietaire', 'section_proprietaire', 'Section Proprietaire'],
        'criticite': ['criticite', 'Criticite', 'Criticité'],
        'disponibilite': ['disponibilite', 'Disponibilite', 'Disponibilité'],
        'fiabiliteIntegrite': ['fiabiliteIntegrite', 'fiabilite_integrite', 'Fiabilite Integrite', 'FiabiliteIntegrite'],
        'processSafety': ['processSafety', 'process_safety', 'Process Safety', 'ProcessSafety'],
        'systeme': ['systeme', 'Systeme', 'Système'],
        'equipmentId': ['equipmentId', 'equipment_id', 'Equipment ID', 'EquipmentID']
      };

      const parser = createReadStream(processedFilePath).pipe(
        parse({
          columns: (headers) => {
            ...
            return headers.map((header: string) => {
              ...
              // Find the standard header name for this column
              for (const [standardName, variations] of Object.entries(expectedHeaders)) {
                if (variations.some(v => v.toLowerCase() === cleanHeader.toLowerCase())) {
                  return standardName;
                }
              }
              
              return cleanHeader; // If no match, keep original (cleaned) header
            });
          },
      ...
      for await (const record of parser) {
        try {
          ...
          if (!record || Object.keys(record).length === 0) {
            ...
            continue;
          }          // Clean up the record - headers should already be normalized
          const cleanRecord = {
            numEquipement: record.numEquipement || '',
            description: record.description || '',
            dateDetectionAnomalie: this.cleanDateField(record.dateDetectionAnomalie || ''),
            descriptionEquipement: record.descriptionEquipement || '',
            sectionProprietaire: record.sectionProprietaire || '',
            criticite: record.criticite || '',
            disponibilite: record.disponibilite || '',
            fiabiliteIntegrite: record.fiabiliteIntegrite || '',
            processSafety: record.processSafety || '',
      ...
      if (processedFilePath !== filepath) {
        try {
          ...
          logger.debug(`Removed temporary preprocessed file: ${processedFilePath}`);
        } catch (unlinkError) {
          logger.warn(`Failed to remove temporary file: ${unlinkError instanceof Error ? unlinkError.message : 'Unknown error'}`);
        }
      }
  ...
  private cleanDateField(dateStr: string): string {
    ...
    try {
      ...
      // Handle dates with missing time component
      if (dateStr.match(/^\d{4}-\d{2}-\d{2}$/)) {
        return `${dateStr} 00:00:00`;
      }
      
      // Fix dates with incomplete time formats
      if (dateStr.match(/^\d{4}-\d{2}-\d{2} \d{1,2}:\d{1,2}$/)) {
        return `${dateStr}:00`;
      }
  ...
  // Helper method to preprocess CSV content to fix known issues before parsing
  private async preprocessCsvFile(filepath: string): Promise<string> {
    try {
      // Read the file content
      const content = await fs.readFile(filepath, 'utf8');
      
      // Replace line breaks within quoted fields (a common source of parsing errors)
      // This regex looks for quoted fields and replaces any line breaks within them
      ...
      await fs.writeFile(tempFilePath, fixedContent);
      
      return tempFilePath;
    } catch (error) {
      logger.warn(`Failed to preprocess CSV file: ${error instanceof Error ? error.message : 'Unknown error'}`);
      return filepath; // Return original filepath if preprocessing fails
    }
  }
